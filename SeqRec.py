...
class SeqRec():
    ...
    def play_list_WAVs(self, _list, isi):                                            # Loops through a list of WAVs and plays them with 
        try:                                                                                # a given inter-stimulus interval (isi)
            for WAV in _list:
                ...
                mySound.play()                                                     # actually play the sound
                core.wait(mySound.getDuration()+isi)                               # You need the duration of the sound here, 
    ...
    def play_one_level(self, level):                                            # Play one level of sequences
        random.shuffle(level)                                                   # do the shuffle
        self.levelList=[]                                                       # create an empty list
        for seq in level:                                                       # pull out one sequence
            self.play_list_WAVs(seq[1:],self.mainISI)                                    # the the list, but starting from the second element,
            ...
            core.wait(beep.getDuration()+self.mainISI)                                   # wait to collect responses
    ...
    def testing_phase(self):
        ...
        while i < self.testCutOff:
            ...
            mySound.play()                                                     # play the sound object 
            core.wait(mySound.getDuration())                                   # wait until the sound is through playing
            ...
            innerCircle.draw()
            self.win.flip()    
            core.wait(self.testISI)                                          # wait before we play next WAV file
